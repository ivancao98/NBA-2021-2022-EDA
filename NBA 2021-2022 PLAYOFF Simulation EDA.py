...
# Before setting up the format, I am going to rank each team in each conference and pull the top 8 teams
WesternConference = pergameaverages.loc[pergameaverages['Conference'] == 'Western'].nlargest(8, 'Win').reset_index()
EasternConference = pergameaverages.loc[pergameaverages['Conference'] == 'Eastern'].nlargest(8, 'Win').reset_index()
playoffteams = pd.concat([WesternConference, EasternConference])
...
regularseasonplayoffteamstats = pd.concat([WesternConference, EasternConference])
...
# after simulating the playoffs 10000 times, the Phoenix Suns wins 15.56% of the time, second is Dallas Mavericks winning 11.29%. 
# The least likely team to win the NBA Finals are the Denver Nuggets and the Chicago Bulls.
...
# With this plot, we can see that the Phoenix Suns make the NBA Finals 25.6% of the 10,000 simulations, the Dallas Mavericks make the NBA Finals 18.8% of the time.
# The teams least likely to make the NBA Finals are the Denver Nuggets and the Chicago Bulls making it 2.1% and 4% of the times respectively.
...
# With this plot, we can see that the Golden State Warriors have the highest chance of winning the NBA Finals if they made it, they end up winning 63.7% of the time
# if they make the NBA Finals the Minesotta timberwolves wub 61.3% if they make it to the NBA Finals. 
# The least likely team to win NBA Finals when making it is the Chicago Bulls and the Cleveland Cavaliers winning 24.8% and 32.7% respectively.